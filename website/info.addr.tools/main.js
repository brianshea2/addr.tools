...
const dnsLookup = (name, type, { signal }) => fetchOk(`https://addr.tools/dns/${name}/${type}`, { signal }).then(r => r.json())
...
const geoLookup = (ip, { signal }) => fetchOk(`https://ip.addr.tools/${ip.toRange(ip.is4() ? 24 : 56).start}`, { signal })
  .then(r => r.json())
